sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.p.nodes <- model$expandNodeNames(paste("y.p[",g,",",i,",",1:J,"]"))
    y.c.nodes <- model$expandNodeNames(paste("y.c[",g,",",i,",",1:J,"]"))
    kern.nodes <- model$expandNodeNames(paste("kern[",g,",",i,",",1:J,"]"))
    pd.p.nodes <- model$expandNodeNames(paste("pd.p[",g,",",i,",",1:J,"]"))
    pd.c.nodes <- model$expandNodeNames(paste("pd.c[",g,",",i,",",1:J,"]"))
  ...
  run <- function() {
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        lp_initial_y.p <- model$getLogProb(y.p.nodes)
        lp_initial_y.c <- model$getLogProb(y.c.nodes)
        #update proposed s
        ...
        model$calculate(pd.p.nodes) #update pd.p nodes
        model$calculate(pd.c.nodes) #update pd.c nodes
        lp_proposed_y.p <- model$calculate(y.p.nodes) #get proposed y logProb
        lp_proposed_y.c <- model$calculate(y.c.nodes) #get proposed y logProb
        lp_initial <- lp_initial_s + lp_initial_y.p + lp_initial_y.c
        lp_proposed <- lp_proposed_s + lp_proposed_y.p + lp_proposed_y.c
