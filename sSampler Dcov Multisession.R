sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.nodes <- model$expandNodeNames(paste("y[",g,",",i,",",1:J,"]"))
    kern.nodes <- model$expandNodeNames(paste("kern[",g,",",i,",",1:J,"]"))
    pd.nodes <- model$expandNodeNames(paste("pd[",g,",",i,",",1:J,"]"))
  ...
  run <- function() {
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        lp_initial_y <- model$getLogProb(y.nodes)
        #update proposed s
        ...
        lp_proposed_y <- model$calculate(y.nodes) #get proposed y logProb
        lp_initial <- lp_initial_s + lp_initial_y
        lp_proposed <- lp_proposed_s + lp_proposed_y
