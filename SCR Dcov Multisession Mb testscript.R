#Trap-specific behavioral response in SCR observation model. No Ramsey obsmod here.
#simulation scenario set up to vary M, J, and K by session to make sure that functionality works

library(nimble)
library(coda)
source("sim.SCR.Dcov.Mb.R") #single session data simulator
source("sim.SCR.Dcov.Multi.Mb.R") #multisession wrapper that pastes together session-level data
source("init.SCR.Dcov.Multi.Mb.R") #formats data, constants, etc for nimble
source("NimbleModel SCR Dcov Multisession Mb.R") #model file
source("NimbleFunctions SCR Dcov Multisession Mb.R") #custom nimble functions and updates
source("sSampler Dcov Multisession Mb.R") #custom activity center update
...
#one row per session, in columns, first and then subsequent capture prob
p0 <- matrix(c(0.25,0.5),N.session,2,byrow=TRUE)
...
data <- sim.SCR.Dcov.Multi.Mb(N.session=N.session,D.beta0=D.beta0,D.beta1=D.beta1,D.cov=D.cov,InSS=InSS,
                  p0=p0,sigma=sigma,K=K,X=X,
...
nimbuild <- init.SCR.Dcov.Multi.Mb(data,M)
...
Niminits <- list(z=nimbuild$z.init,N=rowSums(nimbuild$z.init),s=nimbuild$s.init,
                 p0=matrix(0.5,N.session,2),sigma=rep(1,N.session),
                 D0=rowSums(nimbuild$z.init)/(rowSums(nimbuild$InSS)*nimbuild$cellArea),D.beta1=rep(0,N.session))

#constants for Nimble
constants <- list(N.session=N.session,M=M,J=nimbuild$J,K1D.p=nimbuild$K1D.p,
                  K1D.c=nimbuild$K1D.c,xlim=nimbuild$xlim,ylim=nimbuild$ylim,
                  D.cov=nimbuild$D.cov,cellArea=nimbuild$cellArea,n.cells=nimbuild$n.cells,
                  res=nimbuild$res)

#supply data to nimble
Nimdata <- list(y.p=nimbuild$y.p,y.c=nimbuild$y.c,
                z=nimbuild$z.data,X=nimbuild$X,
...
for(g in 1:N.session){
  for(i in 1:M[g]){
    conf$addSampler(target = paste0("s[",g,",",i,", 1:2]"),
                    type = 'sSampler',control=list(g=g,i=i,J=nimbuild$J[g],
...
for(g in 1:N.session){
  ...
  conf$addSampler(target = c(paste0("p0[",g,",1]"),paste0("sigma[",g,"]")), #p0.USCR
